...
function AskAIPanel({ t, lang, currentLang, speechCode }) {
  ...
  return (
    <div className="bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-950/30 dark:to-green-950/20 border border-emerald-200 dark:border-emerald-800/50 rounded-2xl overflow-hidden ">
      <button type="button" onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between px-5 py-4 text-left">
        <div className="flex items-center gap-3">
          <span className="text-2xl">🤖</span>
          <div>
            <div className="font-bold text-emerald-800 dark:text-emerald-300 text-sm">{t("Ask AI Agri-Expert")}</div>
            <div className="text-xs text-emerald-600 dark:text-emerald-400">{t("Instant answers powered by Gemini AI")}</div>
          </div>
        </div>
        <span className="text-emerald-600 dark:text-emerald-400 text-sm">{open ? "▲" : "▼"}</span>
      </button>
      ...
      {open && (
        <div className="border-t border-green-200 dark:border-green-700">
          ...
          <div className="h-64 overflow-y-auto p-4 space-y-3 bg-white/50 dark:bg-slate-800/50">
            ...
            {loading && <p className="text-xs text-emerald-600 dark:text-emerald-400 pl-1">{t("Thinking…")}</p>}
            <div ref={bottomRef} />
          </div>
          ...
          <div className="flex gap-2 p-3 border-t border-green-100 dark:border-green-800 bg-white dark:bg-slate-800">
            <input value={input} onChange={e => setInput(e.target.value)}
              ...
              disabled={loading}
              className="flex-1 border border-gray-200 dark:border-slate-600 rounded-xl px-3 py-2 text-xs bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 outline-none focus:border-emerald-500" />
